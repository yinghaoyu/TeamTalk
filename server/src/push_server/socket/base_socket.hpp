...
extern "C"
{
   ...
   */
  static S_SOCKET S_CreateSocket(int32_t nAf, int32_t nType, int32_t nProto) { return socket(nAf, nType, nProto); }
   ...
   *  @return 0 success, -1 fail.
   */
  static int32_t S_ShutDown(S_SOCKET nSockFd, int32_t nHow) { return shutdown(nSockFd, nHow); }
   ...
   *  @return 0 success, -1 fail.
   */
  static int32_t S_Listen(S_SOCKET nSockFd, int32_t nNum) { return listen(nSockFd, nNum); }
   ...
   *  @return 0 success, -1 fail.
   */
  static int32_t S_SetTTL(S_SOCKET nSockFd, int32_t nTTL) { return setsockopt(nSockFd, IPPROTO_IP, IP_TTL, (char *) &nTTL, sizeof(nTTL)); }
   ...
   *  @return 0 success, -1 fail.
   */
  static int32_t S_SetTOS(S_SOCKET nSockFd, int32_t nTOS) { return setsockopt(nSockFd, IPPROTO_IP, IP_TOS, (char *) &nTOS, sizeof(nTOS)); }
   ...
   *  @return 0 success, -1 fail.
   */
  static int32_t S_SetReuseAddr(S_SOCKET nSockFd, BOOL bReuse) { return setsockopt(nSockFd, SOL_SOCKET, SO_REUSEADDR, (char *) &bReuse, sizeof(bReuse)); }
   ...
   *  @return 0 success, -1 fail
   */
  static int32_t S_SetNoDelay(S_SOCKET nSockFd, BOOL bNoDelay) { return setsockopt(nSockFd, IPPROTO_TCP, TCP_NODELAY, (char *) &bNoDelay, sizeof(bNoDelay)); }
   ...
   */
  static int32_t S_Send(S_SOCKET nSockFd, void *pBuf, int32_t nBufLen) { return send(nSockFd, pBuf, nBufLen, 0); }
   ...
   */
  static int32_t S_Recv(S_SOCKET nSockFd, void *pBuf, int32_t nBufSize) { return recv(nSockFd, pBuf, nBufSize, 0); }
   ...
   *  @return
   */
  static uint64_t S_ntohll(uint64_t val) { return (((uint64_t) htonl((int32_t)((val << 32) >> 32))) << 32) | (uint32_t) htonl((int32_t)(val >> 32)); }
   ...
   *  @return
   */
  static uint64_t S_htonll(uint64_t val) { return (((uint64_t) htonl((int32_t)((val << 32) >> 32))) << 32) | (uint32_t) htonl((int32_t)(val >> 32)); }
