...
bool OnlineTransferTask::ChangePullState(uint32_t user_id, int file_role)
{
  ...
  do
  {
    ...
    {
      // log("Invalid state, valid state is kTransferTaskStateReady or kTransferTaskStateWaitingSender or kTransferTaskStateWaitingReceiver, but state is %d",
      //  state_);
      break;
    }
...
OfflineTransferTask *OfflineTransferTask::LoadFromDisk(const std::string &task_id, uint32_t user_id)
{
  ...
  if (fp)
  {
    ...
    if (size == sizeof(file_header))
    {
      ...
      {
        offline = new OfflineTransferTask(file_header.get_task_id(), file_header.get_from_user_id(), file_header.get_to_user_id(), file_header.get_file_name(),
      ...
      {
        // log("Offile file size by task_id=%s, user_id=%u, header_file_size=%u, disk_file_size=%u", task_id.c_str(), user_id, file_header.get_file_size(),
        // file_size);
      }
    }
    else
